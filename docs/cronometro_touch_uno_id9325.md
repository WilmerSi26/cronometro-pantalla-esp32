...
## Parpadeo del cronometro

El Arduino UNO es limitado para redibujar pantallas TFT en paralelo. Para reducir el parpadeo, el cronometro touch actualiza el tiempo cada `100 ms` y ya no borra todo el bloque del numero en cada ciclo.

La constante que controla esto es:

```cpp
TIMER_UPDATE_MS = 100
```

Si quieres que se actualice mas rapido, prueba `50`, pero puede volver el parpadeo.
